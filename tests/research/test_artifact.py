...
def test_artifact_assembly_and_state_anchor() -> None:
    ...
    artifact = ResearchArtifact(
        schema_version="P19.3",
        state_hash=STATE_B,
        events=(child, root),
        observations=({"event_id": root.event_id, "text": "signal observed"},),
        hypotheses=({"event_id": root.event_id, "claim": "advance is plausible"},),
        actions=({"event_id": child.event_id, "operation": "advance"},),
        outcomes=({"state_hash": STATE_B, "result": "accepted"},),
        reproducibility={"environment": "research-sandbox", "seed": 42},
    )
...
def test_canonical_serialization_is_order_invariant_for_mappings() -> None:
    root, child = make_events()
    left = ResearchArtifact(
        schema_version="P19.3",
        state_hash=STATE_B,
        events=(root, child),
        observations=({"event_id": root.event_id, "a": 1, "b": 2},),
        reproducibility={"seed": 42, "environment": "sandbox"},
    )
    right = ResearchArtifact(
        schema_version="P19.3",
        state_hash=STATE_B,
        events=(child, root),
        observations=({"b": 2, "a": 1, "event_id": root.event_id},),
        reproducibility={"environment": "sandbox", "seed": 42},
    )
...
def test_artifact_is_immutable() -> None:
    root, child = make_events()
    artifact = ResearchArtifact(schema_version="P19.3", state_hash=STATE_B, events=(root, child))
...
def test_unknown_event_lineage_is_rejected() -> None:
    ...
    with pytest.raises(ResearchArtifactError, match="unknown event_id"):
        ResearchArtifact(
            ...
            state_hash=STATE_B,
            events=(root, child),
            observations=({"event_id": "f" * 64, "text": "orphan"},),
        )
...
def test_state_provenance_must_match_artifact_anchor() -> None:
    ...
    with pytest.raises(ResearchArtifactError, match="must match"):
        ResearchArtifact(
            ...
            state_hash=STATE_B,
            events=(root, child),
            outcomes=({"state_hash": STATE_A, "result": "wrong anchor"},),
        )
...
def test_semantic_records_require_exactly_one_provenance_anchor() -> None:
    ...
    with pytest.raises(ResearchArtifactError, match="exactly one"):
        ResearchArtifact(
            ...
            state_hash=STATE_B,
            events=(root, child),
            observations=({"event_id": root.event_id, "state_hash": STATE_B},),
        )
...
def test_state_anchor_must_exist_in_causal_history() -> None:
    ...
    with pytest.raises(ResearchArtifactError, match="anchored"):
        ResearchArtifact(schema_version="P19.3", state_hash=replay_hash({"state": "C"}), events=(root,))
...
def test_payloads_and_reproducibility_are_canonicalizable() -> None:
    ...
    artifact = ResearchArtifact(
        ...
        state_hash=STATE_B,
        events=(root, child),
        observations=({"event_id": root.event_id, "nested": {"x": [1, 2]}},),
        reproducibility={"seed": 42, "tags": ["linux", "python"]},
    )
...
def test_artifact_identity_changes_when_research_content_changes() -> None:
    ...
    first = ResearchArtifact(
        ...
        state_hash=STATE_B,
        events=(root, child),
        hypotheses=({"event_id": root.event_id, "claim": "A"},),
    )
    second = ResearchArtifact(
        schema_version="P19.3",
        state_hash=STATE_B,
        events=(root, child),
        hypotheses=({"event_id": root.event_id, "claim": "B"},),
    )
