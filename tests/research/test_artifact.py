...
def test_canonical_serialization_is_order_invariant_for_mappings() -> None:
    ...
    left = ResearchArtifact(
        ...
        reproducibility={"seed": 42, "environment": "sandbox", "state_hash": STATE_B},
    )
    right = ResearchArtifact(
        ...
        reproducibility={"environment": "sandbox", "state_hash": STATE_B, "seed": 42},
    )
...
def test_artifact_is_immutable() -> None:
    root, child = make_events()
    artifact = ResearchArtifact(
        schema_version="P19.3", state_hash=STATE_B, events=(root, child), reproducibility=repro()
    )
...
def test_state_anchor_must_exist_in_causal_history() -> None:
    ...
    with pytest.raises(ResearchArtifactError, match="anchored"):
        ResearchArtifact(
            schema_version="P19.3",
            state_hash=replay_hash({"state": "C"}),
            events=(root,),
            reproducibility=repro(),
        )
...
def test_reproducibility_metadata_requires_lineage() -> None:
    ...
    with pytest.raises(ResearchArtifactError, match="must match"):
        ResearchArtifact(
            ...
            state_hash=STATE_B,
            events=(root, child),
            reproducibility={"state_hash": STATE_A, "seed": 42},
        )
...
def test_payloads_and_reproducibility_are_canonicalizable() -> None:
    ...
    artifact = ResearchArtifact(
        ...
        reproducibility={"state_hash": STATE_B, "seed": 42, "tags": ["linux", "python"]},
    )
