...
def _base_question(): return {"question_hash": "q-001", "status": "GAP", "formulation": "Which trace discriminates A from B?"}
def _base_evidence(): return {"result_hash": "r-001", "claim_hash": "c-001", "status": "SUPPORTED"}
def _registry(): return {"questions": {"q-001": _base_question()}, "evidence": {"r-001": _base_evidence()}, "claims": {"c-001": {"claim_hash": "c-001", "status": "SUPPORTED", "evidence_refs": ["r-001"]}}}
def _plan(**overrides):
    registry = overrides.pop("registry", None) or _registry()
    payload = {"objective": "Acquire observations that distinguish the unresolved alternatives.", "strategy": PlanningStrategy.DISCRIMINATE.value, "status": PlanningStatus.CANDIDATE.value, "question_refs": ["q-001"], "evidence_refs": ["r-001"], "conditions": {"parameter": "X", "levels": ["A", "B"]}, "candidate_outcomes": ["trace_A", "trace_B"], "experiment_objective": "Observe whether the empirical trace differs between A and B.", "selection_rationale": {"basis": "reduces unresolved uncertainty", "question_refs": ["q-001"]}, "hypothesis_refs": []}
    payload.update(overrides)
    return make_plan(registry=registry, **payload)
def _tamper(record, **changes):
    data = dict(record.export()); data.update(changes); return data
...
def test_gate_09_runtime_independent_identity(): assert all(k not in _plan().export() for k in ("timestamp", "uuid", "pid", "hostname", "environment"))
...
def test_gate_17_recursive_provenance_preserved():
    p = _plan(); assert "question_refs" in p.provenance and "evidence_refs" in p.provenance
...
def test_gate_28_selection_deterministic():
    a = _plan(selection_rationale={"basis": "reduces unresolved uncertainty", "question_refs": ["q-001"]}); b = _plan(selection_rationale={"question_refs": ["q-001"], "basis": "reduces unresolved uncertainty"}); assert a.plan_hash == b.plan_hash
def test_gate_29_equivalent_hash(): assert _plan().plan_hash == _plan().plan_hash
def test_gate_30_distinct_designs_distinct_hash(): assert _plan(conditions={"parameter": "X", "levels": ["A", "B"]}).plan_hash != _plan(conditions={"parameter": "Y", "levels": ["A", "B"]}).plan_hash
...
def test_gate_35_cross_runtime_bytes():
    def normalize(x):
        if hasattr(x, "items"): return {k: normalize(v) for k, v in x.items()}
        if isinstance(x, (tuple, list)): return [normalize(v) for v in x]
        return x
    a = json.dumps(normalize(_plan().export()), sort_keys=True, separators=(",", ":"), ensure_ascii=False)
    b = json.dumps(normalize(_plan().export()), sort_keys=True, separators=(",", ":"), ensure_ascii=False)
    assert a == b
