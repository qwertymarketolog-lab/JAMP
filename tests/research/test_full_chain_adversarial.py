...
class Artifact:
    ...
    def verify(self):
        ...
        if expected != self.artifact_hash:
            raise AuditIntegrityError(f"{self.kind} integrity failure")
        return True

    def export(self):
        return {"artifact_hash": self.artifact_hash, "parent_hash": self.parent_hash,
                "payload": list(self.payload), "trace_hash": self.trace_hash, "state_hash": self.state_hash}
...
def mk(kind, parent, payload=("x",), trace=None, state=None):
    ...
    h = hashlib.sha256(repr(material).encode()).hexdigest()
    return Artifact(kind, h, parent, tuple(payload), trace, state)
...
def chain():
    ...
    for kind in ("question", "plan", "execution", "result", "interpretation", "consensus", "revision"):
        a = mk(kind, parent)
        out.append(a)
        parent = a.artifact_hash
    return tuple(out)


def session_for(items):
    kinds = ("question", "plan", "execution", "result", "interpretation", "claim", "consensus", "revision")
    refs = [SessionArtifact(a.kind if a.kind != "claim" else "claim", a.artifact_hash) for a in items]
    return ResearchSession.create(metadata={"purpose": "P21.4"}, artifacts=refs, status="OPEN")


def audit():
    items = chain()
    return FullChainAudit.create(items, session=session_for(items))


def test_gate_01_full_chain_construction():
    assert audit().stage_names == tuple(x.value for x in RuntimeStage) 

def test_gate_02_p19_p20_p21_integration_surface():
    a = audit(); assert a.integration_layers == ("P19", "P20", "P21")

def test_gate_03_content_addressed_objects():
    assert all(re.fullmatch(r"[0-9a-f]{64}", x.artifact_hash) for x in chain())

def test_gate_04_full_provenance_reconstruction():
    assert len(audit().provenance) == 7

def test_gate_05_runtime_identity_absent():
    assert not any(k in json.dumps(audit().export()).lower() for k in ("timestamp", "pid", "hostname", "memory_address"))

def test_gate_06_session_identity_preserved():
    a = audit(); assert a.session.verify() and a.session_hash == a.session.session_hash

def test_gate_07_causal_state_anchors_preserved():
    a = audit(); assert all(x.trace_hash and x.state_hash for x in a.artifacts)
...
def test_gate_10_full_chain_reproducible():
...
def _must_reject(items):
    with pytest.raises((RuntimeIntegrityError, AuditIntegrityError, ValueError)):
        FullChainAudit.create(items, session=session_for(items))
...
def test_gate_20_independent_verification():
    assert audit().verify_independent() is True
...
def test_gate_24_trajectory_divergence():
    a = chain(); b = list(a); b[0] = mk("question", None, ("alternate",)); b[1] = mk("plan", b[0].artifact_hash); assert a[3].trace_hash != b[3].trace_hash
...
def test_gate_29_payload_injection():
    _must_reject(_tampered(0, payload=("__injected__", {"timestamp": 1})))
...
def test_gate_32_metadata_injection():
    with pytest.raises(ValueError): ResearchSession.create(metadata={"timestamp": 1}, artifacts=(), status="OPEN")
