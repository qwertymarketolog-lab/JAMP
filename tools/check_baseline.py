...
def main() -> int:
    ...
    if not args.baseline:
        print(
            "ERROR: JAMP_BASELINE_SHA/--baseline is required; no baseline is invented.",
            file=sys.stderr,
        )
        return 2
    ...
    if unstaged or untracked:
        print(
            "ERROR: unstaged or untracked changes are present; "
            "stage or remove them before publishing.",
            file=sys.stderr,
        )
        if unstaged:
