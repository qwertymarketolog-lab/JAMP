...
def scan(text: str) -> list[Token]:
    ...
    while i < len(text):
        ...
        if ch.isspace():
            ...
            if end < 0:
                raise BSLTokenizationError("unterminated block comment")
            i = end + 2
        elif ch == chr(34):
            start = i
            i += 1
            while i < len(text):
                ...
                raise BSLTokenizationError("unterminated string literal")
        elif _identifier_start(ch):
            start = i
            i += 1
            while i < len(text) and _identifier_part(text[i]):
                i += 1
            value = text[start:i]
            upper = value.upper()
